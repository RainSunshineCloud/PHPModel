...
class ValidateFunc
{
    ...
    public static function tinyint($val,$min = null,$max = null,$flags = 0) : bool
    {
        $max = $max < 127 ? $max: 127;
        $min = $min > -128 ? $min: 127;
        return self::int($val,$min,$max,$flags);
    }

    public static function uTinyint($val,$min = null,$max = null,$flags = 0) : bool
    {
        $max = $max < 255 ? $max: 255;
        $min = $min > 0 ? $min: 0;
        return self::int($val,$min,$max,$flags);
    ...
    /**
     * 精确数值
     * @param  numeric     $val   传入值
     * @param  numeric     $min   最小值
     * @param  numeric     $max   最大值
     * @param  dest        $max   小数点后的位数
     */
    public static function decimal($val,$min = null,$max = null,$dest = '2',$flags = 0) : bool
    {
        return (Helper::strlen(strchr($val, '.')) < $desc + 1) && self::float($val,$min,$max,$flags);
     ...
     * @param  dest        $max   小数点后的位数
     */
    public static function uDecimal($val,$min = null,$max = null,$dest = '2',$flags = 0)
    {
        $min = $min < 0 ? 0 : $min;
        return (Helper::strlen(strchr($val, '.')) < $desc + 1) && self::float($val,$min,$max,$flags);
    }
